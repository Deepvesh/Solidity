...
function calculateCurrentState() private view returns (bool) {
    ...
    uint256 totalVote = approve + reject;

    // Calculate the threshold for passing the proposal
    uint256 passThreshold = totalVote / 2;

    // Check if the number of approvals exceeds the pass threshold
    if (approve > passThreshold) {
